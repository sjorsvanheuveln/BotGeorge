...
class Game {
  ...
  welcomeMessage(event) {
    const opponent = event.white.name === this.name ? event.black.name : event.white.name;
    const reply = opponent === 'ansjovis86' ? 'Hello my Master. We meet again :D.' : `Hi ${opponent}! You can type !commands to see my commands.`;
    this.api.chat(this.gameId, 'player', reply);
  }
